...
pthread_cond_t fd_limit; // Condition variable for buffer not empty
...
int main(int argc, char *argv[])
{
    ...
    if (pthread_barrier_init(&startBarrier, NULL, num_workers + 1) != 0) { // +1 for the manager thread
        perror("Failed to initialize barrier");
        exit(EXIT_FAILURE);
    }

    if (pthread_barrier_init(&endBarrier, NULL, num_workers + 1) != 0) { // +1 for the manager thread
        perror("Failed to initialize barrier");
        exit(EXIT_FAILURE);
    }
    ...
    printf("Total directories copied including main directory: %d\n", directory_copied);
...
void signal_handler(int sig)
{
    if (sig == SIGINT)
    {
        printf("Received SIGINT. Please be patient and wait. Exiting...\n");
